...
int main(int argc, char **argv){
  ...
  */
  ...
  //ros::Publisher pub_costs1 = n.advertise<std_msgs::Float64>("target_costs_test",1000);
  //ros::Publisher pub_distribution = n.advertise<std_msgs::Float64MultiArray>("target_distribution",1000);
  ...
  //std_msgs::Float64 target_costs1_msg;

  /*
  Eigen::Vector2d costs(3,1);
  Eigen::Vector2d quality(3,1);
  Eigen::Vector2d utility()
  */
  ...
  while (ros::ok()){
    ...
    if((flag_callback_targets && flag_callback_odom) && !flag_target_assigned){
      ...
      //target_costs1_msg.data = sqrt(std::pow((odom_ptr.pose.pose.position.x)-(targets_poses[0].position.x),2.0) + std::pow((odom_ptr.pose.pose.position.y)-(targets_poses[0].position.y),2.0) + std::pow((odom_ptr.pose.pose.position.z)-(targets_poses[0].position.z),2.0));
    }
    ...
    if(flag_target_assigned){
      ...
      //pub_costs1.publish(target_costs1_msg);
    }
